...
async def retrieve_events(events_request: EventsRequest, events_service: EventsService):
    events = await events_service.get_events(events_request)
    return events


@router.get("", response_model=list[Event])
async def get_events(events_request: Annotated[Any, Depends(EventsRequest)],
                     events_service: EventsService = Depends(get_events_service)) -> list[Event]:
    return await retrieve_events(events_request, events_service)
...
async def get_events_but_silly(events_request: EventsRequest,
                               events_service: EventsService = Depends(get_events_service)) -> list[Event]:
    return await retrieve_events(events_request, events_service)
